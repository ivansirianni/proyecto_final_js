...
let categoria5 = []
const ver_categoria5 = async() =>{
    const response = await fetch("https://api.escuelajs.co/api/v1/categories/5/products")
    const data = await response.json()    
    for (let items of data){        
        let nueva_categoria5 = new Cat5(items.id, items.title, items.images, items.description, items.price)
        categoria5.push(nueva_categoria5)
    }   
    localStorage.setItem("cat_fornitures", JSON.stringify(categoria5))    
}
...
function funcion_mostrar_destacados(array){
    ...
    array.forEach((items)=>{
        let nueva_cat5 = document.createElement("div")
        nueva_cat5.innerHTML = `<div class="card mb-3 text-light bg-dark" style="max-width: 540px;">
        <div class="row g-0">
          <div class="col-md-4">
        ...
        div_categorias.append(nueva_cat5)
  ...
  btn_mostrar_catalogo.addEventListener("click", ()=>{   
        funcion_mostrar_destacados(categoria5)
