...
let categoria4 = []
const ver_categoria4 = async() =>{
    const response = await fetch("https://api.escuelajs.co/api/v1/categories/4/products")
    const data = await response.json()    
    for (let items of data){        
        let nueva_categoria4 = new Cat4(items.id, items.title, items.images, items.description, items.price)
        categoria4.push(nueva_categoria4)
    }   
    localStorage.setItem("cat_fornitures", JSON.stringify(categoria4))    
}
...
function funcion_mostrar_destacados(array){
    ...
    array.forEach((items)=>{
        let nueva_cat4 = document.createElement("div")
        nueva_cat4.innerHTML = `<div class="card mb-3 text-light bg-dark" style="max-width: 540px;">
        <div class="row g-0">
          <div class="col-md-4">
        ...
        div_categorias.append(nueva_cat4)
  ...
  btn_mostrar_catalogo.addEventListener("click", ()=>{   
        funcion_mostrar_destacados(categoria4)
