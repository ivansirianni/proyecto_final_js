...
let categoria3 = []
const ver_categoria3 = async() =>{
    const response = await fetch("https://api.escuelajs.co/api/v1/categories/3/products")
    const data = await response.json()    
    for (let items of data){        
        let nueva_categoria3 = new Cat3(items.id, items.title, items.images, items.description, items.price)
        categoria3.push(nueva_categoria3)
    }   
    localStorage.setItem("cat_fornitures", JSON.stringify(categoria3))    
}
...
function funcion_mostrar_destacados(array){
    ...
    array.forEach((items)=>{
        let nueva_cat3 = document.createElement("div")
        nueva_cat3.innerHTML = `<div class="card mb-3 text-light bg-dark" style="max-width: 540px;">
        <div class="row g-0">
          <div class="col-md-4">
        ...
        div_categorias.append(nueva_cat3)
  ...
  btn_mostrar_catalogo.addEventListener("click", ()=>{   
        funcion_mostrar_destacados(categoria3)
