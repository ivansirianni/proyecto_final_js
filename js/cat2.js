...
let categoria2 = []
const ver_categoria2 = async() =>{
    const response = await fetch("https://api.escuelajs.co/api/v1/categories/2/products")
    const data = await response.json()    
    for (let items of data){        
        let nueva_categoria2 = new Cat2(items.id, items.title, items.images, items.description, items.price)
        categoria2.push(nueva_categoria2)
    }   
    localStorage.setItem("cat_electro", JSON.stringify(categoria2))    
}
...
function funcion_mostrar_destacados(array){
    ...
    array.forEach((items)=>{
        let nueva_cat2 = document.createElement("div")
        nueva_cat2.innerHTML = `<div class="card mb-3 text-light bg-dark" style="max-width: 540px;">
        <div class="row g-0">
          <div class="col-md-4">
        ...
        div_categorias.append(nueva_cat2)
  ...
  btn_mostrar_catalogo.addEventListener("click", ()=>{   
        funcion_mostrar_destacados(categoria2)
